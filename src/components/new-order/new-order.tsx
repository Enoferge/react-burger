import newOrderImage from '@/images/order-success.png';
...
export const NewOrder = ({ order }: TNewOrderProps): React.JSX.Element => {
  return (
    <div className={`${styles.root} pt-4 pb-15`}>
      <span className={'text text_type_digits-large'}>{order?.number ?? '??????'}</span>
      <span className={'text text_type_main-medium mt-8'}>идентификатор заказа</span>
      <img className={'mt-15 mb-15'} alt="Заказ успешно создан" src={newOrderImage} />
